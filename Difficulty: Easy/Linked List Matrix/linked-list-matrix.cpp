...
class Solution {
  public:
    Node* constructLinkedMatrix(vector<vector<int>>& mat) {
       Node* head=NULL;
        Node* nextnode=new Node(mat[0][0]);
        Node* newrow , *newcol , *nextcol;
        int n=mat.size();
        int i=0,j=0;
        head=nextnode;
        ...
        for( i=0;i<n;i++){
            ...
            for( j=0;j<n;j++){
                ...
                if(i+1>=n) {
                    nextnode->down=NULL;}
                if(i+1<n) {
                    newcol=new Node(mat[i+1][j]); 
                    if(j==0) nextcol=newcol;
                    nextnode->down=newcol;}
                if(j+1<n) {
                    newrow=new Node(mat[i][j+1]);
                    nextnode->right=newrow;
